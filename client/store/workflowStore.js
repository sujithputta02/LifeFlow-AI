...
export const useWorkflowStore = create((set, get) => ({
    ...
    fetchHistory: async () => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/v1/history?guestId=${guestId}`);
            if (response.ok) {
    ...
    deleteWorkflow: async (id) => {
        try {
            await fetch(`${API_BASE_URL}/api/v1/history/${id}`, {
                method: 'DELETE',
            });
            set((state) => ({
    ...
    syncGamification: async (points, level, badges) => {
        try {
            await fetch(`${API_BASE_URL}/api/v1/gamification/update`, {
                method: 'POST',
    ...
    fetchProfile: async () => {
        try {
            const guestId = get().guestId;
            if (!guestId) return;
            const res = await fetch(`${API_BASE_URL}/api/v1/gamification/${guestId}`);
            if (res.ok) {
    ...
    generateWorkflow: async (goal, language) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/v1/generate-workflow`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    verifyStep: async (stepId, stepTitle, stepDescription, proof) => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/v1/verify-step`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
